...
/**
 * Generated class for the ButcheryPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-butchery',
  templateUrl: 'butchery.html'
})
export class ButcheryPage {
  ...
  butchery() {
    ...
    if(!this.states.grade) {
      ...
      alert("'등급' 항목을 선택하세요!");
      return;
    }else if(!this.states.passFlag) {
      this.open('passFlag');
      alert("'도축 검사' 항목을 선택하세요!");
      return;
    }
    ...
    var process = {
      ...
      passFlag: this.states.passFlag
    };

    this.ready.then(() => {
      this.websocket.send(JSON.stringify(process));
      console.log("Process : " + process);
      alert("도축 신고가 완료되었습니다!\n이력번호 : " + this.myPigId);
    });
  }
